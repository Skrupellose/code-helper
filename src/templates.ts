...
5. 页面测试文档写入 \`${config.directories.resultDoc}/<中文功能名>/手工测试.md\`。
6. 最终计划必须同时具备总纲、分层顺序、组件计划、页面计划、验收标准、状态跟踪、阻塞回归入口和当前推进建议。
7. 生成顺序必须是：目标与约束、P0/P1 总纲、依赖顺序、目录或模块策略、基础阶段计划、组件计划、页面计划、执行工作台。
8. 不要一开始直接写细表；先给总纲和阶段边界，再逐步细化。
9. 必须按依赖闭环重排顺序，不按页面直觉排序。
10. 推荐依赖顺序：基础目录、类型定义、API / 请求层、状态与守卫、应用骨架、i18n 最小闭环、通用组件与布局层、组件组、页面、回归与横向升级。
11. 页面不是第一迁移单元；进入页面计划前必须先生成页面到组件依赖总表、组件分组、组件优先级、单组件 checklist 和组件组真实执行顺序。
12. 页面逐页执行表必须包含前置依赖、执行任务、UI 验收、功能验收和完成定义。
13. 所有总览表和明细表统一使用“子任务 / 已完成 / 未完成 / 备注”四列。
14. 备注第一句必须写 \`状态：未开始\`、\`状态：进行中\`、\`状态：部分完成\`、\`状态：被阻塞\` 或 \`状态：已完成\`。
15. 备注必须说明阻塞点、依赖后续流程、回归入口，以及是否允许跳过继续推进。
16. 文档前部必须包含“当前推进建议”，明确现在先做什么、下一步接什么、第一个组件组何时开始、第一个页面何时开始、当前默认执行主线是什么。
17. 页面相关验收必须拆成 UI 验收和功能验收，并生成严格手工测试文档；工具自身只执行纯逻辑测试。
18. 文档只有满足可直接推进、可记录阻塞、可恢复上下文、可支持回归时，才算成型。`
    },
    {
...
3. 执行 \`npx code-helper archive <中文功能名>\` 时，将 \`${config.directories.planDoc}/<中文功能名>.md\`、\`${config.directories.resultDoc}/<中文功能名>/\`、\`${config.directories.statusDoc}/<中文功能名>-状态.md\` 移入对应 archive 目录。
...
description: 当用户要求“更新记忆”“优化记忆”“沉淀规则”“整理 AGENTS.md”“整理 CLAUDE.md”“同步 AGENTS.md 和 CLAUDE.md”“拆分项目规则文档”“把当前变更写入记忆”时必须使用。该 skill 同时识别 AGENTS.md 与 CLAUDE.md，把项目记忆维护为轻量入口文档 + code-helper-docs/user-rules/ 专题规则文档；如果 code-helper-docs/ 或 code-helper-docs/user-rules/ 不存在则按需创建，避免整份覆盖、重复建档或把入口文件写成大而全的长文档。新功能或重构完成后，也用它提醒用户手动触发记忆更新。
---
