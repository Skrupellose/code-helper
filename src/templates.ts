...
4. 页面测试文档写入 \`${config.directories.resultDoc}/<feature>/manual-test.md\`。
...
3. 执行 \`npx code-helper archive <feature>\` 时，将 \`${config.directories.planDoc}/<feature>.md\`、\`${config.directories.resultDoc}/<feature>/\`、\`${config.directories.statusDoc}/<feature>-status.md\` 移入对应 archive 目录。
...
- .agent/plan-doc/<feature>.md：执行计划工作台
- .agent/result-doc/<feature>/implementation.md：阶段或小节点实施记录
- .agent/status-doc/<feature>-status.md：当前状态驾驶舱
- .agent/result-doc/<feature>/manual-test.md：页面手工测试文档
...
- result-doc：实施记录，记录实际改动、验证结论、临时失败、风险和后续。
