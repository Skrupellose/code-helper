...
7. 多 agent 协作时由验证矩阵指定每条命令的唯一默认负责人；同一基线、同一工作树状态和同一命令已有可核验回执时默认复用，只在回执无效、基线变化、结果不稳定、风险覆盖不足或用户明确要求时复跑受影响部分。
8. 会共同写入 \`dist\`、\`coverage\`、缓存、快照、依赖目录或其它共享产物的验证命令必须串行执行，即使被测文件和代码写入范围互不重叠。`
    },
    {
      fileName: "文档归档规范.md",
      content: `# 文档归档规范
...
23. 最终回复前必须输出紧凑进度摘要；计划任务复用 status-doc，直接执行任务按实际 diff、验证和风险生成，recorded 终态只说明已结束。
24. “剩余门禁”只包含当前阻断；当前非阻断和后续优化必须单独列出，不得因此把已满足原始完成定义的节点继续标为进行中。
25. 多 agent 任务收尾时先核对验证回执是否与当前任务 ID、起始基线和 diff 身份一致；有效回执默认复用，只补跑因基线变化、结果不稳定或风险覆盖不足而失效的部分。`
    },
    {
...
7. 任务按风险与复杂度分为 T0-T3：T0 是精确只读查询或微型操作；T1 是边界清晰、局部可逆且可定向验证的任务；T2 是跨模块、根因不确定或适合独立并行的任务；T3 是会改变或绕过安全/权限门禁、处理真实敏感用户数据、执行不可逆迁移、生产发布、外部写操作或其它高影响副作用的任务。领域名称或代码中出现“安全”“权限”“用户数据”“文件下载”等词不能单独判定为 T3。
...
10. 顺序强依赖链优先由同一执行代理负责到底；补充任务优先复用原执行代理。只有任务相互独立、写入范围不重叠、共享验证产物不冲突且合并成本低于并行收益时才并行分发；会共同写入 \`dist\`、\`coverage\`、缓存、快照或依赖目录的命令必须串行。
11. 派发任务的最小必填语义是任务 ID、执行角色与禁止二次转派、任务目标和完成条件、允许写入范围、关键约束、返回内容；执行身份与转派边界优先由工具父任务、调用关系或角色元数据承载，只有元数据不可用或不能可靠传递时，派发提示才必须明确“你现在是执行子代理”。T2/T3 或共享脏工作树还要记录起始基线、验证矩阵和共享副作用资源。
12. 同一基线、同一工作树状态和同一验证命令默认只执行一次。执行者返回包含任务 ID、基线、diff 身份、精确命令、工作目录、退出码、结果摘要、共享副作用资源和未验证范围的回执；主会话默认复用，只在回执无效、基线变化、结果不稳定、风险覆盖不足或用户明确要求时复跑受影响部分。
...
21. 单个逻辑交付点默认最多触发 4 个子代理任务：T0/T1 默认不派发，T2 默认一个实现子代理且最多两个独立实现工作包，T3 默认一个实现子代理和一个独立复核子代理；预算是上限，不要求用满。
22. 默认只进行一轮独立完整审查；修复后的逐项复审只核对原 finding 和直接回归。只有新的 P0/安全证据、实现范围实质扩大或用户明确要求时，才增加额外完整审查。
23. 实现开始前固定原始完成定义；finding 按当前阻断、当前非阻断、后续优化分类。原始完成定义满足且当前阻断关闭后必须停止自动扩修。
24. T3 的独立复核是强制门禁；未关闭 P0/P1 导致预算不足时停止扩张并请求用户决定，不得用预算上限绕过复核。
25. 节点开始、完成定义变化、关键实现完成、复审结束和最终回复前使用统一进度摘要；普通搜索、单次命令和微型补丁不制造流水式播报。
26. 派发任务统一使用 accepted、running、waiting_input、blocked、completed、cancelled、superseded 状态。等待超时不等于失败；重新派发前必须检查任务状态、工作树和已有产物，取消或替换时明确标记，迟到结果不得直接采用。
27. 协作前按实际工具判断子代理、并行、角色元数据、会话复用、状态查询和中断能力，不根据平台注册目录猜测；缺少某项能力时使用对应串行、显式角色、完整交接、证据核验或忽略迟到结果的回退，但不得放宽 T3 独立隔离。`
    }
  ];
}
