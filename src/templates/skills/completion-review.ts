...
export const completionReviewSkillTemplate: SkillTemplate = {
  ...
  content: `---
name: code-helper-completion-review
description: 当 agent 完成可独立验收的逻辑交付点并准备最终回复，或进行提交前检查、切换任务、询问是否归档、询问是否更新记忆，以及用户要求“检查是否完成”“继续下一个任务”“功能收尾”时必须使用。搜索、读取、单次命令或单个补丁等微型步骤不单独触发；真实功能变更在最终回复前仍必须触发。该 skill 要按 active、archived、mixed 生命周期读取任务文档并判断下一步。
---
...
在一个可独立验收的逻辑交付点完成后，避免 agent 未经收尾直接进入最终总结、提交或切换任务。微型执行步骤不重复触发，但真实功能变更在最终回复前必须判断是否完成，再决定继续开发、更新过程文档、询问更新记忆、询问归档或选择下一个任务。
...
5. 当前逻辑交付点未完成时，继续当前功能；不要询问归档，不要引导新任务。
6. 当前逻辑交付点完成但功能整体未完成时，更新实施记录、计划文档状态和 status-doc 的下一个执行节点。
