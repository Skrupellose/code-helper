...
status-doc 必须在“当前执行节点”后提供可直接复制到对话的“进度摘要”，固定包含：
...
6. 后续优化
7. 下一步
8. Agent 使用，例如 2/4；存在独立复审时同时记录复审轮次

主会话只在节点开始、完成定义变化、关键实现完成、复审结束和最终回复前更新并播报摘要，不为搜索、单次命令或微型补丁制造流水账。后续优化不得混入剩余门禁，只有直接阻止当前完成定义的事项才能列为门禁。
