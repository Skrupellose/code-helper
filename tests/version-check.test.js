...
test("formatOutdatedVersionMessage 优先推荐 npx @latest，并补充菜单快捷升级", () => {
  // 版本提示用于交互菜单路径：须先给出不依赖本地安装的 @latest 命令，
  // 再说明存在 package.json 时快捷项会安装或升级本地开发依赖，避免误写成必须预先安装。
  const message = formatOutdatedVersionMessage("0.1.3", "0.1.4").join("\n");

  assert.match(message, /发现 code-helper 新版本：0\.1\.4（当前 0\.1\.3）/);
  assert.match(message, /优先执行：npx @skrupellose\/code-helper@latest update/);
  assert.match(message, /存在 package\.json/);
  assert.match(message, /安装或升级本地开发依赖/);
  assert.match(message, /调用新版 update/);
  assert.match(message, /主菜单顶部「安装或升级到最新版本」/);
  // 不应再把预先 npm i -D 或仅菜单项当作唯一升级路径。
  assert.doesNotMatch(message, /npm i -D @skrupellose\/code-helper@latest/);
});
