...
test("createCompletionReview 在 mixed 部分归档时双侧探测文档并提示整理", async () => {
  // plan 仅在 archive、result/status 仍在 active 且文档/节点齐全时，reviewStatus 必须是 mixed，绝不能是 ready-to-archive。
  ...
  try {
    ...
    // 三侧文档齐全且状态全完成时，结论固定为 mixed，不得升为 ready-to-archive。
    assert.equal(review.reviewStatus, "mixed");
    assert.notEqual(review.reviewStatus, "ready-to-archive");
    assert.equal(review.documents.plan.exists, true);
    assert.match(review.documents.plan.relativePath, /archive/);
    assert.equal(review.documents.result.exists, true);
    assert.equal(review.documents.status.exists, true);
...
test("createCompletionReview 在 mixed 且文档齐全状态全完成时仍为 mixed 而非 ready-to-archive", async () => {
  // active + archive 双侧都有完整 plan/result/status，且标记全完成，仍不得 ready-to-archive / 切换下一任务。
  const root = await mkdtemp(join(tmpdir(), "code-helper-completion-mixed-complete-"));

  try {
    await initializeProject({ projectRoot: root });
    await mkdir(join(root, "code-helper-docs/plan-doc/archive"), { recursive: true });
    await mkdir(join(root, "code-helper-docs/result-doc/双侧齐全/"), { recursive: true });
    await mkdir(join(root, "code-helper-docs/result-doc/archive/双侧齐全"), { recursive: true });
    await mkdir(join(root, "code-helper-docs/status-doc/archive"), { recursive: true });

    // active 侧完整三文档
    await writeFile(
      join(root, "code-helper-docs/plan-doc/双侧齐全.md"),
      "# 双侧齐全\n\n状态：已完成\n",
      "utf8"
    );
    await writeFile(
      join(root, "code-helper-docs/result-doc/双侧齐全/实施记录.md"),
      "# 实施记录\n\n状态：已完成\n",
      "utf8"
    );
    await writeFile(
      join(root, "code-helper-docs/status-doc/双侧齐全-状态.md"),
      "# 状态\n\n## 当前执行节点\n\n状态：已完成\n\n## 子计划队列\n\n状态：已完成\n",
      "utf8"
    );

    // archive 侧同名文档，触发 taskStatus=mixed
    await writeFile(
      join(root, "code-helper-docs/plan-doc/archive/双侧齐全.md"),
      "# 双侧齐全归档\n\n状态：已完成\n",
      "utf8"
    );
    await writeFile(
      join(root, "code-helper-docs/result-doc/archive/双侧齐全/实施记录.md"),
      "# 归档实施记录\n\n状态：已完成\n",
      "utf8"
    );
    await writeFile(
      join(root, "code-helper-docs/status-doc/archive/双侧齐全-状态.md"),
      "# 归档状态\n\n## 当前执行节点\n\n状态：已完成\n\n## 子计划队列\n\n状态：已完成\n",
      "utf8"
    );

    const review = await createCompletionReview(root, "双侧齐全");

    assert.equal(review.taskStatus, "mixed");
    assert.equal(review.reviewStatus, "mixed");
    assert.notEqual(review.reviewStatus, "ready-to-archive");
    assert.notEqual(review.reviewStatus, "needs-work");
    assert.equal(review.shouldAskArchive, true);
    assert.equal(review.shouldSelectNextTask, false);
    assert.ok(review.recommendations.some((item) => item.includes("不得视为 ready-to-archive")));
    assert.ok(review.requiredConfirmations.some((item) => item.includes("mixed")));
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("createCompletionReview 在 mixed 文档不全时仍为 missing-docs", async () => {
  // mixed 时保留 missing-docs 可操作性：plan 在 archive，但 result/status 全缺 → missing-docs，不强制 shouldAskArchive。
  const root = await mkdtemp(join(tmpdir(), "code-helper-completion-mixed-missing-"));

  try {
    await initializeProject({ projectRoot: root });
    await mkdir(join(root, "code-helper-docs/plan-doc"), { recursive: true });
    await mkdir(join(root, "code-helper-docs/plan-doc/archive"), { recursive: true });
    // active + archive 各有 plan，触发 mixed；故意不写 result/status
    await writeFile(join(root, "code-helper-docs/plan-doc/混合缺文档.md"), "# 混合缺文档\n\n状态：进行中\n", "utf8");
    await writeFile(
      join(root, "code-helper-docs/plan-doc/archive/混合缺文档.md"),
      "# 混合缺文档归档\n\n状态：已完成\n",
      "utf8"
    );

    const review = await createCompletionReview(root, "混合缺文档");

    assert.equal(review.taskStatus, "mixed");
    assert.equal(review.reviewStatus, "missing-docs");
    assert.equal(review.shouldAskArchive, false);
    assert.equal(review.shouldSelectNextTask, false);
    assert.ok(review.recommendations.some((item) => item.includes("mixed") && item.includes("补齐")));
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
