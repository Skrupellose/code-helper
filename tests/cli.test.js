...
test("主菜单按项目准备、工具设置分组展示", () => {
  // 主菜单只保留项目准备与工具设置；任务推进/维护改走 agent skills 与 CLI 子命令。
  const groups = getMainMenuGroups();

  assert.deepEqual(groups.map((group) => group.title), ["项目准备", "工具设置"]);
  assert.deepEqual(
  ...
  // 已移除的任务推进/项目维护入口不得再出现
  assert.equal(groups.some((group) => group.title === "任务推进" || group.title === "项目维护"), false);
  assert.equal(
    groups.flatMap((group) => group.items).some((item) =>
      ["生成任务计划模板", "生成手工测试模板", "检查功能完成情况", "查看任务列表", "归档已完成任务", "检查协作规范"].includes(item.name)
    ),
    false
  );
  assert.equal(groups.flatMap((group) => group.items).some((item) => item.value === "5"), false);
...
test("主菜单 raw mode 选项包含不可选分组和功能说明", () => {
  ...
  assert.equal(options.find((option) => option.label === "【工具设置】")?.disabled, true);
  assert.equal(options.some((option) => option.label === "【任务推进】"), false);
  assert.equal(options.some((option) => option.label === "【项目维护】"), false);
  assert.equal(options.some((option) => option.label === "" && option.disabled), true);
  assert.ok(options.some((option) => option.value === "2" && option.label.includes("  2. 功能管理")));
  assert.ok(options.some((option) => option.value === "2" && option.label.includes("应用或取消项目级 Skills")));
  assert.ok(options.some((option) => option.value === "3" && option.label.includes("管理项目 Skills")));
  assert.ok(options.some((option) => option.value === "4" && option.label.includes("管理 Hooks")));
  assert.equal(options.some((option) => option.value === "5" || option.label.includes("更新 code-helper 本地资产")), false);
...
test("主菜单布局格式区分标题、功能名和说明", () => {
  ...
  // groups[1] 为「工具设置」，首项为功能管理（value 2）
  const item = getMainMenuGroups()[1].items[0];

  assert.equal(formatMainMenuGroupTitle("工具设置"), "【工具设置】");
  assert.equal(formatMainMenuGroupTitle("项目准备"), "【项目准备】");
  assert.match(formatMainMenuSelectItemLabel(item), /^ {3}2\. 功能管理 +应用或取消项目级 Skills/);
  assert.deepEqual(formatMainMenuTextItemLines(item), [
    "   2. 功能管理",
    "      应用或取消项目级 Skills、Agent hooks 和 Git hook"
  ]);
});
