...
test("首次 init 创建内置规则，state.json 含 packageVersion", async () => {
  ...
  try {
    ...
    assert.match(gitCommitRuleContent, /# Git 提交信息格式规范/u);
    assert.match(gitCommitRuleContent, /普通提交统一使用：\s*\x60<type>\(<scope>\): <subject>\x60/u);
    assert.match(gitCommitRuleContent, /Breaking change 提交统一使用：\s*\x60<type>\(<scope>\)!: <subject>\x60/u);
    assert.match(gitCommitRuleContent, /scope 必填/u);
    assert.match(gitCommitRuleContent, /type 和 scope 使用英文，subject 和 body 默认使用中文/u);
    assert.match(gitCommitRuleContent, /BREAKING CHANGE:/u);
    assert.match(gitCommitRuleContent, /任何 footer 前都必须与上一段空一行/u);
...
test("update 会为旧项目补齐新增的 Git 提交信息格式规范", async () => {
  // 旧项目的 state 可能已存在，但尚未安装后来新增的内置规则；update 必须按模板清单补齐缺失文件。
  const root = await mkdtemp(join(tmpdir(), "code-helper-rules-git-commit-update-"));

  try {
    await writeFile(join(root, "AGENTS.md"), "# Agents\n", "utf8");
    await initializeProject({ projectRoot: root, skillRegistrationTargets: [] });

    const rulePath = join(root, "code-helper-docs/user-rules/Git提交信息格式规范.md");
    await rm(rulePath);
    await writeFile(join(root, "AGENTS.md"), "# Agents\n", "utf8");
    ...
    assert.ok(
      result.operations.some(
        (operation) => operation.path.endsWith("Git提交信息格式规范.md") && operation.action === "created"
      )
    );
    assert.match(content, /<type>\(<scope>\): <subject>/u);
    assert.match(content, /scope 必填/u);
    assert.match(agents, /Git 提交信息格式规范/u);
    assert.match(agents, /Git提交信息格式规范\.md/u);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test("默认刷新保留缺少受控指纹的内置同名用户文件", async () => {
  // 新版本可能新增与用户文件同名的内置规则；没有 state 或历史指纹时只能同步入口，不能整文件覆盖。
  const root = await mkdtemp(join(tmpdir(), "code-helper-rules-unknown-same-name-default-"));
  ...
  try {
    ...
    const statePath = join(root, ".code-helper/state.json");
    const state = JSON.parse(await readFile(statePath, "utf8"));
    delete state.ruleTemplateFingerprints["Git提交信息格式规范.md"];
    await writeFile(statePath, `${JSON.stringify(state, null, 2)}\n`, "utf8");

    const unknownSameName = "# Git 提交信息格式规范\n\n## 用户自定义正文\n\n不得覆盖。\n";
    await writeFile(rulePath, unknownSameName, "utf8");

    const result = await updateProject(root);
    const after = await readFile(rulePath, "utf8");
    const afterState = JSON.parse(await readFile(statePath, "utf8"));
    const operation = result.operations.find((item) => item.path.endsWith("Git提交信息格式规范.md"));

    assert.match(after, /## 用户自定义正文/u);
    assert.match(after, /不得覆盖/u);
    assert.ok(operation);
    assert.match(operation.message, /缺少受控指纹.*保留正文/u);
    assert.equal(afterState.ruleTemplateFingerprints["Git提交信息格式规范.md"], undefined);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test("force 刷新跳过缺少受控指纹的内置同名用户文件", async () => {
  // 即使显式 force，也只能覆盖 state 或历史指纹能证明归属的路径；同名不能作为所有权证据。
  const root = await mkdtemp(join(tmpdir(), "code-helper-rules-unknown-same-name-force-"));
  ...
  try {
    ...
    const unknownSameName = "# Git 提交信息格式规范\n\n用户自己的同名规则，force 也必须逐字保留。\n";
    await writeFile(rulePath, unknownSameName, "utf8");

    const result = await updateProject(root, { refreshRules: true });
    const after = await readFile(rulePath, "utf8");
    const afterState = JSON.parse(await readFile(statePath, "utf8"));
    const operation = result.operations.find((item) => item.path.endsWith("Git提交信息格式规范.md"));

    assert.equal(after, unknownSameName);
    assert.ok(operation);
    assert.equal(operation.action, "skipped");
    assert.match(operation.message, /缺少受控指纹.*强制刷新已跳过/u);
    assert.equal(afterState.ruleTemplateFingerprints["Git提交信息格式规范.md"], undefined);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
